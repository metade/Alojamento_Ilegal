...
module AlIlegal
  module Data
    ...
    def self.prepare_alojamentos_locais!
      date = Date.today
      return if File.exist?("data_sources/Estabelecimentos_de_Alojamento_Local-#{date}.csv")
      ...
      URI.open(csv_url) do |remote_file|
        File.write("data_sources/Estabelecimentos_de_Alojamento_Local-#{date}.csv", remote_file.read)
      end
    end
    ...
    def self.prepare_airbnb!
      ...
      return if File.exist?("data_sources/listings-#{date}.csv")
      ...
      URI.open(airbnb_data_url) do |remote_file|
        Zlib::GzipReader.wrap(remote_file) do |gz|
          File.write("data_sources/listings-#{date}.csv", gz.read)
        end
      end
    end
  end
